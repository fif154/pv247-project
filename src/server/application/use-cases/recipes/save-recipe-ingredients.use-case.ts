...
export type SaveRecipeIngredientsInput = z.infer<
  typeof saveRecipeIngredientsSchema
>;
...
export const saveRecipeIngredientsUseCase =
  ...
  async (input: SaveRecipeIngredientsInput, tx?: Transaction) => {
    ...
    await recipeIngredientsRepository.deleteRecipeIngredientsByRecipeId(
      input.recipeId,
      tx
    );
    ...
    const combinedIngredients =
      recipeIngredientsService.combineIngredients(ingredientsToCreate);

    return await recipeIngredientsRepository.createRecipeIngredients(
      combinedIngredients,
      tx
    );
  };

export type ISaveRecipeIngredientsUseCase = ReturnType<
  typeof saveRecipeIngredientsUseCase
>;
