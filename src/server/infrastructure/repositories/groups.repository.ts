...
export class GroupsRepository implements IGroupsRepository {
    ...
    async editGroup(group: Group): Promise<Group | undefined> {
        const query = db.update(groups).set(group).where(eq(groups.id, group.id)).returning();
    ...
    async removeGroup(id: string): Promise<boolean> {
        const query = db.update(groups).set({deletedAt: new Date().toISOString()}).where(eq(groups.id, id)).returning();
