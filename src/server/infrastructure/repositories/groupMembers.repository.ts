...
export class GroupMembersRepository implements IGroupMembersRepository {
    ...
    async removeUserFromAllGroups(userId: string): Promise<boolean> {
        const query = db.update(groupMembers).set({deletedAt: new Date().toISOString()}).where(eq(groupMembers.userId, userId)).returning();
    ...
    async removeUserFromGroup(userId: string, groupId: string): Promise<boolean> {
        const query = db.update(groupMembers).set({deletedAt: new Date().toISOString()}).where(and(eq(groupMembers.userId, userId),eq(groupMembers.groupId, groupId))).returning();
    ...
    async removeAllUsersFromGroup(groupId: string): Promise<boolean> {
        const query = db.update(groupMembers).set({deletedAt: new Date().toISOString()}).where(eq(groupMembers.groupId, groupId)).returning();
    ...
    async addUserToGroup(userId: string, groupId: string): Promise<GroupMember> {
        const query = db.insert(groupMembers).values({userId, groupId}).returning();
    ...
    async addUsersToGroup(userIds: string[], groupId: string): Promise<GroupMember[]> {
